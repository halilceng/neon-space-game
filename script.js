document.addEventListener('DOMContentLoaded', () => {
    ...
    const SoundManager = {
        ...
        playTone: (freq, type, duration, vol = 0.1, detune = 0) => {
            if (!audioCtx) return;
            if (audioCtx.currentTime > 0 && Math.random() < 0.1) return; // Ses optimizasyonu
    ...
    function startWave() {
        ...
        enemiesToKill = 5 + (wave * 3); // Zorluk Dengesi
    ...
    function updateMainMenu() { document.getElementById('menu-high-score').innerText = saveData.highScore;
    ...
    function gameLoop() {
        ...
        if (isSlowMoActive) { slowMoEnergy -= 0.5; if (slowMoEnergy <= 0) toggleSlowMo(false); } else if (slowMoEnergy < 100) { slowMoEnergy += 0.2; }
        fuelBar.style.width = slowMoEnergy + '%';

        let playerTimeScale = isSlowMoActive ? 0.6 : 1.0;

        // --- YENİ HAREKET FİZİĞİ (KAYMAYI ENGELLEYEN KISIM) ---
        // Hızlanma (Acceleration) değerini düşürdük: 0.8 -> 0.5
        if (keys.ArrowLeft) speedX -= 0.5 * playerTimeScale;
        if (keys.ArrowRight) speedX += 0.5 * playerTimeScale;

        // Sürtünme (Friction) değerini düşürdük: 0.92 -> 0.80 (Daha çabuk durur)
        speedX *= 0.80;

        // Max Hız Sınırı (Clamping)
        if (speedX > 2.5) speedX = 2.5;
        if (speedX < -2.5) speedX = -2.5;
        ...
        // Dönüş açısını biraz azalttık ki çok savruluyor gibi görünmesin
        player.style.transform = `translateX(-50%) rotate(${speedX * -4}deg)`;
    ...
    function spawnEnemy() {
        ...
        let type = 'enemy-ship';
        // Zorluk Ayarı: İlk dalgalarda sadece basit düşmanlar
        if (wave > 2 && rand < 0.2) type = 'kamikaze';
        else if (wave > 4 && rand < 0.4) type = 'interceptor';
    ...
    function updateBombUI() { let s = ''; for (let i = 0; i < bombs; i++) s += '💣 ';
        bombCountText.innerText = s || 'YOK'; }

    function triggerGlitch() { glitchOverlay.classList.add('glitch-active');
